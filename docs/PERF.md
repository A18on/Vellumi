...
差值法测量(先确认系统内无其他 WebContent 进程,基线 0):
...
**每增加一个文档约 48 MB。** RSS 会把跨进程共享的框架页重复计入,所以这是上界。

据此做出的决策:**不拆分 JS bundle**。6.4 MB 的产物里 mermaid 子树占约 49%,
把它改成按需 `import()` 需要 1-2 天;但决定性的数字是**增量**成本,48 MB 远低于
先前设定的 80 MB 阈值,惰性加载至多再省下其中一小部分。首个文档的固定成本
(507 MB)主要来自 app 进程与首个 WebContent,不是 bundle 解析。
